...
class TestAgentBody extends KinematicBody:
...
func test_initialization():
	...
	assert_true(is_instance_valid(movement_system.agent_body), "It should have a valid reference to its parent agent body.")
...
func test_accelerates_when_aligned():
	...
	assert_true(agent_body.current_velocity.length() > 0.0, "Velocity should increase when accelerating while aligned.")
	assert_true(agent_body.current_velocity.z < 0, "Velocity should be in the local forward direction (negative Z).")
...
func test_does_not_accelerate_when_not_aligned():
	...
	assert_almost_eq(agent_body.current_velocity.length(), 0.0, 0.001, "Velocity should not increase when not aligned.")
...
func test_braking_reduces_speed_faster_than_deceleration():
	...
	assert_true(speed_after_brake < speed_after_decel, "Braking should be stronger than natural deceleration.")
...
func test_rotation_turns_towards_target():
	...
	assert_true(final_dot > initial_dot, "Agent should turn to be more aligned with the target direction.")
