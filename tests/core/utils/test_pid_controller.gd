...
func test_integral_limit_clamping():
	...
	assert_almost_eq(pid.update(1.0, 1.0), 10.0, 0.001, "Output should use the clamped integral value.")
	pid.reset()
	pid.update(-15.0, 1.0) # integral would be -15, but clamped to -10
	assert_almost_eq(pid.integral, -10.0, 0.001, "Integral should be clamped to its negative limit.")
