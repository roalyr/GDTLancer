...
func test_spawn_agent_successfully():
	...
	assert_not_null(agent_body.init_data, "The agent's `initialize` method should have been called.")
	assert_eq(agent_body.init_data.template, template, "Agent was initialized with the correct template.")
	...
	assert_eq(captured_args[0], agent_body, "Signal should include the spawned agent body as the first argument.")
...
func test_spawn_player_finds_entry_point():
	...
	assert_almost_eq(player_body.global_transform.origin.x, expected_pos.x, 0.001, "Player spawn X position should be correct.")
	assert_almost_eq(player_body.global_transform.origin.y, expected_pos.y, 0.001, "Player spawn Y position should be correct.")
	assert_almost_eq(player_body.global_transform.origin.z, expected_pos.z, 0.001, "Player spawn Z position should be correct.")
