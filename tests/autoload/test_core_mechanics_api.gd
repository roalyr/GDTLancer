...
func test_action_check_focus_bonus_calculation():
	...
	assert_eq(result_2fp.roll_total, result_2fp.dice_sum - 1 + (2 * Constants.FOCUS_BOOST_PER_POINT), "Total includes mod and focus bonus.")
...
func test_action_check_focus_spending_clamp():
	...
	assert_eq(result_over.focus_bonus, Constants.FOCUS_MAX_DEFAULT * Constants.FOCUS_BOOST_PER_POINT, "Focus bonus uses clamped value.")
...
func test_action_check_result_tier_boundaries():
	...
	assert_lt(18 + mod_fail + 0, Constants.ACTION_CHECK_FAIL_THRESHOLD, "Check setup: Max possible roll should be < FailThreshold")
	...
	assert_true(3 + mod_crit + fp_crit * Constants.FOCUS_BOOST_PER_POINT >= Constants.ACTION_CHECK_CRIT_THRESHOLD, "Check setup: Min possible roll should be >= CritThreshold")
	...
	# Check setup condition using assert_true with comparison
	assert_true(3 + mod_nofail + fp_nofail * Constants.FOCUS_BOOST_PER_POINT >= Constants.ACTION_CHECK_SWC_THRESHOLD, "Check setup: Min possible roll should be >= SwCThreshold")
	...
	assert_lt(18 + mod_nocrit + fp_nocrit * Constants.FOCUS_BOOST_PER_POINT, Constants.ACTION_CHECK_CRIT_THRESHOLD, "Check setup: Max possible roll should be < CritThreshold")
