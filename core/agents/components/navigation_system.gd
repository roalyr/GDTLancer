...
const APPROACH_DECELERATION_START_DISTANCE_FACTOR = 3.0  # Start PID deceleration when distance <= this * desired_stop_dist
...
func initialize_navigation(nav_params: Dictionary, move_sys_ref: Node):
	...
	if is_instance_valid(_orbit_pid):
		var pid_kp = nav_params.get("orbit_kp", 0.01)
	...
	if is_instance_valid(_approach_pid):
		...
		var ap_ki = nav_params.get("approach_ki", 0.01)
		var ap_kd = nav_params.get("approach_kd", 0.3)
	...
	if is_instance_valid(_move_to_pid):
		...
		var mt_ki = nav_params.get("move_to_ki", 0.01)
		var mt_kd = nav_params.get("move_to_kd", 0.3)
