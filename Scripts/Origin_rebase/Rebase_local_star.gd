...
func _ready():
	# ============================= Connect signals ===========================
	p.signals.connect("sig_exited_local_space_star", self, "is_exited_local_space_star")
	p.signals.connect("sig_entered_local_space_star", self, "is_entered_local_space_star")
	# =========================================================================
...
func is_entered_local_space_star(zone):
	...
	if p.local_space_star.has_node("Scenes"):
		var scene_names = []

		for c in p.local_space_star.get_node("Scenes").get_children():
			scene_names.append(c)
			
		var message = "Local space '"+p.local_space_star.get_name() \
	...
	p.local_space_star.global_transform = zone.global_transform

	# Reparent scenes from global to local space.
	zone.remove_child(scenes)
	p.local_space_star.add_child(scenes)
...
func is_exited_local_space_star(zone):
	...
	p.local_space_star.remove_child(scenes)
