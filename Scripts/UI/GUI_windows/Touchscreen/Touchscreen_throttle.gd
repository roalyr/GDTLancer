...
func recenter_throttle():
	...
	p.input_throttle.throttle_y_abs = throttle_base.rect_size.x/2

func handle_throttle():
	# Process virtual throttle input.
	if p.common_game_options.touchscreen_mode:
		if p.ui.throttle_held:
			throttle.position.y = p.input_throttle.throttle_y_abs-100
		else:
			# Recenter throttle.
			if throttle.position != Vector2(-20, 70):
				throttle.position.x = -20
				throttle.position.y = throttle_base.rect_size.y/2-100
				
				# Reset throttle input coords to prevent jumping.
				p.input_throttle.throttle_y_abs = throttle_base.rect_size.y/2
