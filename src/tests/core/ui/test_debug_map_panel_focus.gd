...
func before_each():
	...
	GameState.current_sector_id = "sector_star_lywin"
...
func test_opening_panel_keeps_camera_at_origin_and_shifts_sector_markers():
	...
	var lywin_template = TemplateDatabase.locations["sector_star_lywin"]
	assert_not_null(elace_template, "Expected sector_star_elace template to load for map-anchor test.")
	assert_not_null(lywin_template, "Expected sector_star_lywin template to load for pivot test.")
	_panel_instance._toggle_panel()
	var map_content = _panel_instance.get_node("Panel/VBoxContainer/MapArea/ViewportContainer/Viewport/MapContent")
	var lywin_marker = map_content.get_node_or_null("Sector_sector_star_lywin")
	var elace_marker = map_content.get_node_or_null("Sector_sector_star_elace")
	assert_not_null(lywin_marker, "Expected sector_star_lywin marker to exist after opening the panel.")
...
func test_opening_panel_aligns_backdrop_sector_stars_with_sector_markers():
	var lywin_template = TemplateDatabase.locations["sector_star_lywin"]
	assert_not_null(lywin_template, "Expected sector_star_lywin template to load for backdrop-alignment test.")
	_panel_instance._toggle_panel()
	yield(get_tree(), "idle_frame")
	var map_content = _panel_instance.get_node("Panel/VBoxContainer/MapArea/ViewportContainer/Viewport/MapContent")
	var lywin_marker = map_content.get_node_or_null("Sector_sector_star_lywin")
	var elace_marker = map_content.get_node_or_null("Sector_sector_star_elace")
	assert_not_null(lywin_marker, "Expected sector_star_lywin marker to exist for backdrop alignment test.")
	...
	var star_lywin_a = backdrop.get_node_or_null("%s/Star Lywin Sprite" % star_root_path)
	var star_elace = backdrop.get_node_or_null("%s/Star Elace Sprite" % star_root_path)
	assert_not_null(star_lywin_a, "Expected Star Lywin Sprite to exist in the dedicated map backdrop.")
	...
	if star_lywin_a and star_elace:
		assert_eq(
			...
			lywin_marker.global_transform.origin,
			"Star Lywin Sprite in the backdrop should land on the same map-space position as the Lywin A sector marker."
		)
		assert_eq(
